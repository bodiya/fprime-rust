...
use fprime_comp::{
    ...
    MsgDispatchStatus, OutputPort, PingPort, PortRef, QueueFullPolicy, SchedPort, TlmGlue,
};
use fprime_config::{
...
use fprime_fw::{Endianness, LinearBuffer, LogSeverity, SerBuf, SerBufAny, fw_assert};
...
const MSG_TYPE_CYCLE_IN: FwEnumStoreType = 1;
...
impl ActiveRateGroup {
    ...
    pub fn cycle_in(self: &Arc<Self>, port_num: FwIndexType) -> PortRef<dyn CyclePort> {
        PortRef::new(Arc::new(CycleInAdapter { comp: self.clone() }), port_num)
    }
...
/// `CycleIn` adapter: `CycleIn_preMsgHook` (set the slip flag on the
/// sender thread) + envelope + enqueue with the `drop` policy.
struct CycleInAdapter {
    comp: Arc<ActiveRateGroup>,
}

impl CyclePort for CycleInAdapter {
    fn invoke(&self, port_num: FwIndexType, cycle_start: &RawTime) {
        // CycleIn_preMsgHook: runs on the SENDER thread BEFORE enqueue.
        self.comp.cycle_started.store(true, Ordering::Relaxed);

        let mut buf = LinearBuffer::<MSG_SIZE>::new();
        let status = msg::write_envelope_header(&mut buf, MSG_TYPE_CYCLE_IN, port_num);
        fw_assert!(status.is_ok(), status as i32);
        // RawTime wire format: [sec u32][nsec u32] BE (8 bytes).
        let status = buf.serialize(cycle_start, Endianness::Big);
        fw_assert!(status.is_ok(), status as i32);
        // `drop` policy: a full queue silently discards (counted).
        let _ =
            self.comp
                .active
                .queued
                .send_message(&buf, CYCLE_IN_PRIORITY, QueueFullPolicy::Drop);
    }
}

/// `PingIn` adapter (default/assert policy).
...
impl ComponentDispatch for ActiveRateGroup {
    fn dispatch_message(
        ...
        match msg_type {
            MSG_TYPE_CYCLE_IN => {
                let mut cycle_start = RawTime::default();
                if !buf.deserialize(&mut cycle_start, Endianness::Big).is_ok() {
                    return MsgDispatchStatus::Error;
                }
