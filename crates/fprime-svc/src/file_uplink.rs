...
//!
//! This module also hosts the `Os::SandboxedFile` / `Os::FilePathUtils` port
//! ([`SandboxedFile`], [`resolve_path`], [`resolve_from_cwd`],
//! [`check_containment`]) that `Svc::FileDownlink` shares. They belong in
//! `fprime-os`, which is owned by an earlier wave and not writable here;
//! move them when that crate is next touched.
...
use fprime_config::{
    FILE_NAME_STRING_SIZE, FwChanIdType, FwEnumStoreType, FwEventIdType, FwIdType, FwIndexType,
    FwPacketDescriptorType, FwQueuePriorityType, FwSizeType,
};
...
use fprime_os::{File, filesystem};
use fprime_utils::cfdp::Checksum;
use std::sync::{Arc, Mutex};

// ---------------------------------------------------------------------------
// Os::FilePathUtils (ported here; see the module header)
// ---------------------------------------------------------------------------
